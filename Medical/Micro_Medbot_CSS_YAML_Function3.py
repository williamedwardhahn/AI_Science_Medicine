...
def present_options(request, question, responses):
    options_html = ''.join(
        f'<a href="/procedure/{urllib.parse.quote(option)}"><button>{option}</button></a>' for option in responses.keys())
...
def handle_procedure(request, response=None):
    if response is None:  # If no response provided, begin procedure
        current_state.clear()
        response_html = present_options(request, "What is the nature of the medical emergency?", emergency_procedures)
